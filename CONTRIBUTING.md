...
- `internal/plex`, `internal/tautulli`: HTTP API clients for each upstream.
  `internal/plex` is a thin adapter over the shared `cplieger/plexapi` client
  (which owns its transport, retry, and timeouts), mapping responses into
  `remap` types. `internal/tautulli` wraps its reads in `cplieger/httpx`
  (`httpx.GetBytes`: bounded exponential-backoff retry on 429/5xx, honoring
  `Retry-After`); mutating Tautulli commands call the bare client directly and
  are never retried.
...
- **Structured logging is key-value only.** `sloglint` runs with `kv-only`,
  so every `slog` call must use `slog.Info("msg", "key", val)` form, with no
...
`main()` itself has no direct test (thin signal/scheduler/trigger wrapper
validated by the Docker healthcheck); `main_test.go` covers its
`finishTrigger` exit-code mapping. Add tests for new logic and keep
